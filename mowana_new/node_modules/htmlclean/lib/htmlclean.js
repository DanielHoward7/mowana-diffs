...
var PHRASING_ELEMENTS = require('./phrasing-elements'),
  ...
  PTN_ATTR_NAME = '[^\\s/=>"\'<]+',
  PTN_ATTR = '[^\\S\\f]+' + PTN_ATTR_NAME +
    '(?:[^\\S\\f]*=[^\\S\\f]*(?:\'[^\']*\'|"[^"]*"|[^\\s>"\'<=`]+))?',
  PTN_ATTR_CAP = '[^\\S\\f]+(' + PTN_ATTR_NAME +
    ')(?:[^\\S\\f]*=[^\\S\\f]*(?:(\')([^\']*)\'|(")([^"]*)"|([^\\s>"\'<=`]+)))?',
  PTN_ATTRS = '(?:' + PTN_ATTR + ')*',
  PTN_TAG_NAME = '[^\\s/>]+',
  PTN_IN_TAG = '[^\\S\\f]*/?[^\\S\\f]*' + PTN_TAG_NAME + PTN_ATTRS + '[^\\S\\f]*/?[^\\S\\f]*',
...
module.exports = function(html, options) {
  ...
  */
  html = html
    ...
    .replace(new RegExp('(<[^\\S\\f]*\\?[^\\S\\f]*xml' + PTN_ATTRS + '[^\\S\\f]*\\?[^\\S\\f]*>)', 'ig'),
      function(str, p1) { return addUnprotectedText(p1); })
    // <% ... %>, <? ... ?>, <?php ... ?>
    .replace(/(<[^\S\f]*(\%|\?)[^]*?\2[^\S\f]*>)/g, // Not [^>]. '>' may be included.
      function(str, p1) { return addProtectedText(p1); })
    // <?php ... (Other than above)
    .replace(/(<[^\S\f]*\?[^\S\f]*php\b[^]*)/ig,
      function(str, p1) { return addProtectedText(p1); })
    // <jsp: ... >
    .replace(new RegExp('(<[^\\S\\f]*jsp[^\\S\\f]*:' + PTN_ATTRS + '[^\\S\\f]*>)', 'ig'),
      function(str, p1) { return addProtectedText(p1); })
    // <!--# ... -->
  ...
  html = html.replace(new RegExp('(<[^\\S\\f]*(textarea|script|style|pre)' +
      PTN_ATTRS + '[^\\S\\f]*>)([^]*?)(<[^\\S\\f]*/[^\\S\\f]*\\2' +
      PTN_ATTRS + '[^\\S\\f]*>)', 'ig'),
    function(str, startTag, tagName, innerHtml, endTag) {
      ...
      if (innerHtml !== '') {
        if (tagName.toLowerCase() === 'pre') { // Allow nesting tags.
          splitHtml = '';
          innerHtml = innerHtml.replace(new RegExp('([^]*?)(<' + PTN_IN_TAG + '>)', 'g'),
  ...
  // Attributes of tags
  html = html.replace(new RegExp('<(' + PTN_IN_TAG + ')>', 'g'),
    function(str, tagInner) {
      tagInner = tagInner.replace(/^[^\S\f]+|[^\S\f]+$/g, ''); // Not .trim() that removes \f.
      var tagName =
        (tagInner.match(new RegExp('^/?[^\\S\\f]*(' + PTN_TAG_NAME + ')')) || [])[1];
      return '<' + tagInner.replace(new RegExp(PTN_ATTR_CAP, 'g'),
        function(str, attrName, quot1, attrValue1, quot2, attrValue2, attrValue3) {
          var attrValue, quot;
          if (quot1) {
            ...
            quot = quot2;
          } else if (attrValue3) { // Unquoted value must be not empty text
            attrValue = attrValue3;
            quot = '';
          } else { // This means no value
            quot = false;
          }
          return ' ' + attrName + (quot === false ? '' : '=' + quot +
            addProtectedText(
  ...
  // Whitespaces in tags
  html = html.replace(/<([^>]+)>/g, // `>` in attributes were already replaced
    function(str, tagInner) {
      tagInner = tagInner
        .replace(/^ +| +$/g, '') // Not .trim() that removes \f.
